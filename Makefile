...
docker-test: image-test
	$(eval WORKDIR := /go/src/github.com/everoute/everoute)
	docker run --rm -iu 0:0 -w $(WORKDIR) -v $(CURDIR):$(WORKDIR) everoute/unit-test make test
...
docker-cover-test: image-test
	$(eval WORKDIR := /go/src/github.com/everoute/everoute)
	docker run --rm -iu 0:0 -w $(WORKDIR) -v $(CURDIR):$(WORKDIR) everoute/unit-test make cover-test
...
docker-race-test: image-test
	$(eval WORKDIR := /go/src/github.com/everoute/everoute)
	docker run --rm -iu 0:0 -w $(WORKDIR) -v $(CURDIR):$(WORKDIR) everoute/unit-test make race-test
