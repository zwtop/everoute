...
func (c *Controller) deleteRelatedPolicies(indexName, key string) error {
	policies, err := c.crdPolicyLister.ByIndex(indexName, key)
	if err != nil {
	...
	for _, obj := range policies {
		policy := obj.(*v1alpha1.SecurityPolicy)
		err := c.crdClient.SecurityV1alpha1().SecurityPolicies(policy.GetNamespace()).Delete(context.Background(), policy.GetName(), metav1.DeleteOptions{})
		if err != nil && !errors.IsNotFound(err) {
			klog.Errorf("delete policy %+v: %s", policy, err)
			return err
		}
	}
...
func (c *Controller) applyPoliciesChanges(oldKeys []string, new []v1alpha1.SecurityPolicy) error {
	...
	for _, policy := range new {
		policyKey, _ := cache.MetaNamespaceKeyFunc(policy)
		if oldKeySet.Has(policyKey) {
			...
			if exist {
				...
				_, err := c.crdClient.SecurityV1alpha1().SecurityPolicies(policy.GetNamespace()).Update(context.Background(), &policy, metav1.UpdateOptions{})
				if err != nil {
		...
		// create the policy
		_, err := c.crdClient.SecurityV1alpha1().SecurityPolicies(policy.GetNamespace()).Create(context.Background(), &policy, metav1.CreateOptions{})
		if err != nil {
...
func (c *Controller) parseNetworkPolicyRule(rule *schema.NetworkPolicyRule) ([]v1alpha1.SecurityPolicyPeer, []v1alpha1.SecurityPolicyPort, error) {
	...
	case schema.NetworkPolicyRuleTypeIPBlock:
		...
		policyPeers = append(policyPeers, v1alpha1.SecurityPolicyPeer{
			IPBlock: &networkingv1.IPBlock{CIDR: *rule.IPBlock},
		})
