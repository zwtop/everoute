...
var (
	...
	EncodingSchemeMatchCTLabel     = [16]byte{0x30} // EncodingSchemeMicroSegmentationMask << EncodingSchemeXXREG0BitStart
	EncodingSchemeMatchCTLabelMask = [16]byte{0x30} // 1 << EncodingSchemeXXREG0BitStart .. 1 << EncodingSchemeXXREG0BitEnd
...
func (p *PolicyBridge) BridgeInit() {
	...
	if err := p.initFinalActionUpdateTable(sw); err != nil {
		klog.Fatalf("Failed to init final action update table, error: %v", err)
	}
...
func (p *PolicyBridge) initFinalActionUpdateTable(_ *ofctrl.OFSwitch) error {
	...
	policyToLocalPort := p.datapathManager.BridgeChainPortMap[p.localBrName][PolicyToLocalSuffix]
	...
	for _, m := range matches {
		for _, um := range actionMatches {
			for _, et := range ethertypes {
				...
				updateAction, _ := ofctrl.NewConntrackActionWithZoneField(
					true,
					false,
...
type ctCommitDirection struct {
	inputPort uint32
	// originTreatedAct is applied on -rpl/origin commit path.
	originTreatedAct *openflow13.NXActionRegLoad
	// rplTreatedAct is applied on +rpl rematch path with reversed direction.
	rplTreatedAct     *openflow13.NXActionRegLoad
	originWorkAction  openflow13.Action
	rplRematchWorkAct openflow13.Action
}

type ctCommitActionSet struct {
	moveFinalActionAct    openflow13.Action
	movePolicyAct         openflow13.Action
	moveRoundNumAct       openflow13.Action
	markOriginSourceAct   openflow13.Action
	markInportAct         openflow13.Action
	resetReplySourceAct   openflow13.Action
	resetReplyInportAct   openflow13.Action
	setSourceWorkAction   openflow13.Action
	setTargetWorkAction   openflow13.Action
	markReplySourceAct    openflow13.Action
	markReplyInportAct    openflow13.Action
	markMSAct             openflow13.Action
	maskEgressTreatedAct  *openflow13.NXActionRegLoad
	maskIngressTreatedAct *openflow13.NXActionRegLoad
}

func newCtCommitActionSet() ctCommitActionSet {
	...
	setSourceWorkAction := openflow13.NewNXActionRegMove(
		WorkPolicyActionXXREG0BitSize,
		WorkPolicyActionXXREG0Bit,
	...
	setTargetWorkAction := openflow13.NewNXActionRegMove(
		WorkPolicyActionXXREG0BitSize,
		WorkPolicyActionXXREG0Bit,
	...
	return ctCommitActionSet{
		moveFinalActionAct:    moveFinalActionAct,
		movePolicyAct:         movePolicyAct,
		moveRoundNumAct:       moveRoundNumAct,
		markOriginSourceAct:   markOriginSourceAct,
		markInportAct:         markInportAct,
		resetReplySourceAct:   resetReplySourceAct,
		resetReplyInportAct:   resetReplyInportAct,
		setSourceWorkAction:   setSourceWorkAction,
		setTargetWorkAction:   setTargetWorkAction,
		markReplySourceAct:    markReplySourceAct,
		markReplyInportAct:    markReplyInportAct,
		markMSAct:             markMSAct,
		maskEgressTreatedAct:  maskEgressTreatedAct,
		maskIngressTreatedAct: maskIngressTreatedAct,
	}
}

func (p *PolicyBridge) newCtCommitDirections(actions ctCommitActionSet) []ctCommitDirection {
	return []ctCommitDirection{
		{
			inputPort:         p.datapathManager.BridgeChainPortMap[p.localBrName][PolicyToLocalSuffix],
			originTreatedAct:  actions.maskEgressTreatedAct,
			rplTreatedAct:     actions.maskIngressTreatedAct,
			originWorkAction:  actions.setSourceWorkAction,
			rplRematchWorkAct: actions.setTargetWorkAction,
		},
		{
			inputPort:         p.datapathManager.BridgeChainPortMap[p.localBrName][PolicyToClsSuffix],
			originTreatedAct:  actions.maskIngressTreatedAct,
			rplTreatedAct:     actions.maskEgressTreatedAct,
			originWorkAction:  actions.setTargetWorkAction,
			rplRematchWorkAct: actions.setSourceWorkAction,
		},
	}
}

func newCtCommitOriginFlowMatch(priority uint16) ofctrl.FlowMatch {
	return ofctrl.FlowMatch{
		Priority: priority,
...
func newCtCommitRplRematchFlowMatch(priority uint16) ofctrl.FlowMatch {
	return ofctrl.FlowMatch{
		Priority: priority,
...
func newCtCommitReplyFlowMatch(priority uint16) ofctrl.FlowMatch {
	return ofctrl.FlowMatch{
		Priority: priority,
		CtStates: ctRplTrkState(),
		Regs: []*ofctrl.NXRegister{
...
func (p *PolicyBridge) initCTFlow(_ *ofctrl.OFSwitch) error {
	...
	actions := newCtCommitActionSet()
	originFlowMatch := newCtCommitOriginFlowMatch(MID_MATCH_FLOW_PRIORITY)
	rplRematchFlowMatch := newCtCommitRplRematchFlowMatch(MID_MATCH_FLOW_PRIORITY)
	ethertypes := [...]uint16{protocol.IPv4_MSG, protocol.IPv6_MSG}
	directions := p.newCtCommitDirections(actions)
	...
	for _, ethertype := range ethertypes {
		...
		for _, direction := range directions {
			...
			ctCommitAction, _ := ofctrl.NewConntrackActionWithZoneField(true, false, &ctDropTable, policyCTZoneReg, policyCTZoneRange,
				actions.moveFinalActionAct, actions.movePolicyAct, actions.moveRoundNumAct, // policy numbers
				direction.originWorkAction,                         // src/dst action
				actions.markOriginSourceAct, actions.markInportAct, // inport and origin source bridge
				actions.resetReplySourceAct, actions.resetReplyInportAct, // reset origin source and inport
				actions.markMSAct,          // micro segmentation
				direction.originTreatedAct, // ER-1177 mask the flow processed with policies in egress.
			)
	...
	ctCommitRplAction, _ := ofctrl.NewConntrackActionWithZoneField(true, false, &ctDropTable, policyCTZoneReg, policyCTZoneRange,
		actions.markReplySourceAct, actions.markReplyInportAct, // inport and reply source bridge
		actions.markMSAct, // micro segmentation
	)

	replyFlowMatch := newCtCommitReplyFlowMatch(MID_MATCH_FLOW_PRIORITY)
	replyFlowMatch.Ethertype = protocol.IPv4_MSG
	ctCommitReplyFlow, _ := p.ctCommitTable.NewFlow(replyFlowMatch)
	...
	for _, ethertype := range ethertypes {
		...
		for _, direction := range directions {
			...
			ctRplRematchAction, _ := ofctrl.NewConntrackActionWithZoneField(true, false, &ctDropTable, policyCTZoneReg, policyCTZoneRange,
				actions.moveFinalActionAct, actions.movePolicyAct, actions.moveRoundNumAct, // policy numbers
				direction.rplRematchWorkAct,                            // src/dst action
				actions.markReplySourceAct, actions.markReplyInportAct, // inport and reply source bridge
				actions.markMSAct,       // micro segmentation
				direction.rplTreatedAct, // mark policy treated state
			)
...
func (p *PolicyBridge) initALGFlow(_ *ofctrl.OFSwitch) error {
	...
	// match
	originFlowMatch := newCtCommitOriginFlowMatch(MID_MATCH_FLOW_PRIORITY + FLOW_MATCH_OFFSET)
	rplRematchFlowMatch := newCtCommitRplRematchFlowMatch(MID_MATCH_FLOW_PRIORITY + FLOW_MATCH_OFFSET)
	replyFlowMatch := newCtCommitReplyFlowMatch(MID_MATCH_FLOW_PRIORITY + FLOW_MATCH_OFFSET)
...
func (p *PolicyBridge) UpdateTREndpoint(ep *Endpoint) error {
	...
	if ep.IfaceID != p.TrafficRedirect.Cfg.NicInIfaceID && ep.IfaceID != p.TrafficRedirect.Cfg.NicOutIfaceID {
		klog.Infof("Endpoint %v is not policy bridge tr nic, skip process it for update", ep)
		return nil
	}
	...
	if ep.IfaceID == p.TrafficRedirect.Cfg.NicInIfaceID {
		...
		if ep.PortNo != portIn {
			...
			klog.Infof("Success update policy bridge %s tr in nic to port %v", p.name, ep)
		}
		oldState := p.TrafficRedirect.Info.NicIn.State
		if ep.State != oldState {
			p.TrafficRedirect.Info.NicIn.State = ep.State
			klog.Infof("Update policy bridge %s tr in nic state from %s to %s", p.name, oldState, ep.State)
		}
	}

	if ep.IfaceID == p.TrafficRedirect.Cfg.NicOutIfaceID {
		...
		if ep.PortNo != portOut {
			...
			klog.Infof("Success update policy bridge %s tr out nic to port %v", p.name, ep)
		}
		oldState := p.TrafficRedirect.Info.NicOut.State
		if ep.State != oldState {
			p.TrafficRedirect.Info.NicOut.State = ep.State
			klog.Infof("Update policy bridge %s tr out nic state from %s to %s", p.name, oldState, ep.State)
		}
	}
